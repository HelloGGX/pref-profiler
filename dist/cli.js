...
function parseFlags(args, flagSpec) {
    const flags = {};
    const positionals = [];
    let rest = [];
...
async function cmdDemo(flags) {
    const mode = typeof flags.mode === 'string' ? flags.mode : 'startup';
...
async function cmdReport(flags, positionals) {
    const dir = typeof flags.dir === 'string' ? flags.dir : getOutputDir();
...
async function cmdRun(rest) {
    ...
    const checkpoint = (name) => {
        ...
        if (mark)
            marks.push({ name, startTime: mark.startTime });
    };
    ...
    let lastSample = readProcSample(child.pid);
    ...
    for (const mark of marks) {
        lines.push(formatTimelineLine(mark.startTime - baseline, mark.startTime - prevTime, mark.name, undefined, 10, 9));
    ...
    if (finalSample) {
        ...
        lines.push(`Peak RSS:         ${formatFileSize(finalSample.peakRssBytes)}`);
    }
    else {
